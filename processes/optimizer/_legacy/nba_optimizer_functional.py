...
def _normalize_projections_df(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    def _first_col(candidates):
        ...
        for candidate in candidates:
            for variant in (candidate, candidate.lower(), candidate.upper(), candidate.title()):
    ...
    ownership_candidates = [
        "own_proj", "own%", "ownership", "ownership%", 
        "projected ownership", "own pct", "own_pct"
    ]
...
def _normalize_ownership(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    candidates = [
        "own_proj", "own%", "ownership", "ownership%",
        "projected ownership", "own pct", "own_pct", "ownproj", "proj_own"
    ]
    src = None
...
def find_fuzzy_matches(proj_df: pd.DataFrame, ids_df: pd.DataFrame, 
    ...
    if not fuzzy_df.empty:
        ...
        for _, match in fuzzy_df.head(5).iterrows():
            print(f"  {match['proj_name']} → {match['id_name']} ({match['similarity']:.1%} similar)")
...
def _objective_coeffs_from_players(players: List[Dict], settings, lam: float) -> Dict[str, float]:
    ...
    for p in players:
        ...
        p_eff = _effective_p(own_pct, settings.pivot_p0, settings.shrink_gamma,
                             settings.clamp_min, settings.clamp_max)
        gval = _g_curve(p_eff, settings)
        coeffs[pid] = base - lam * gval
    return coeffs


def _objective_coeffs(projections_df: pd.DataFrame, settings, lam: float) -> Dict[str, float]:
...
def _lineup_raw_projection(lineup_players: List[str], proj_map: Dict[str, float]) -> float:
    """Calculate raw projection total for a lineup (no penalty)"""
    return sum(proj_map.get(pid, 0.0) for pid in lineup_players)


def _measure_offoptimal_pct(P_star: float, lineup_players: List[str], proj_map: Dict[str, float]) -> float:
...
def _solve_with_lambda(lam: float, players: List[Dict], settings, build_and_solve_fn) -> List[str]:
...
def _find_lambda_by_percent(
    ...
    if off_hi < target_pct:
        return (lam_hi, off_hi, hi_players) if abs(off_hi - target_pct) <= abs(off_lo - target_pct) else (lam_lo, off_lo, lo_players)
...
def optimize(projections_df: pd.DataFrame, constraints: Constraints, 
             seed: int, site: SiteType, player_ids_df: Optional[pd.DataFrame] = None,
             engine: str = "cbc") -> List[Lineup]:
    """
    Main optimization function - returns lineups only (for compatibility)
    """
    lineups, _ = optimize_with_diagnostics(projections_df, constraints, seed, site, player_ids_df, engine=engine)
    return lineups


def optimize_with_diagnostics(projections_df: pd.DataFrame, constraints: Constraints, 
                            seed: int, site: SiteType, player_ids_df: Optional[pd.DataFrame] = None,
                            engine: str = "cbc") -> Tuple[List[Lineup], Dict[str, Any]]:
    """
    ...
    projections_df, id_diagnostics = attach_player_ids_if_available(projections_df, site, player_ids_df)
    ...
    try:
        ...
        norm_block = id_diagnostics.setdefault("normalization", {}).setdefault("ownership", {})
        if own_norm:
            norm_block.update(own_norm)
        # Add post-merge columns diff
        _cols_after_merge = sorted(list(projections_df.columns))
        norm_block["cols_diff_post_merge"] = list(set(_cols_before_merge) ^ set(_cols_after_merge))
        try:
            if "own_proj" in projections_df.columns:
                norm_block["own_proj_max_post_merge"] = float(projections_df["own_proj"].max())
        except Exception:
            pass
    except Exception:
        pass
    ...
    try:
        ...
        id_diagnostics.setdefault('constraints_snapshot', {})['ownership_enabled'] = bool(pen_cfg and pen_cfg.enabled)
    except Exception:
        pass
...
• Position validation failed: {id_diagnostics['failed_position_validation']} players""",
    ...
    # Warning mode: inform about compatibility issues  
        ...
        print("   Enable fail-fast mode (require_dk_ids=True) to prevent invalid lineups.")
    ...
    if len(players) == 0:
        raise OptimizerError(
            ...
            details={"proj_min": constraints.proj_min, "available_players": len(projections_df)}
        )
    ...
    if (ownership_penalty and ownership_penalty.enabled and 
        ownership_penalty.mode == "by_percent" and engine in ("cp_sat", "cp_sat_counts")):
        print(f"🎯 Ownership penalty: by % off optimal (target: {ownership_penalty.target_offoptimal_pct:.1%})")

        # Build projection map for off-optimal calculation
        players_for_search = convert_projections_to_players(projections_df, constraints.proj_min)
        ...
        def build_and_solve_fn(coeffs: Dict[str, float]) -> List[str]:
            ...
            if hasattr(base_constraints, 'ownership_penalty') and base_constraints.ownership_penalty:
            ...
            if isinstance(base_constraints.cp_sat_params, dict):
                ...
                params['num_search_workers'] = int(params.get('num_search_workers', 0) or 0)
                base_constraints.cp_sat_params = params
            try:
                from .solvers.cpsat_solver import solve_cpsat_iterative, solve_cpsat_iterative_counts
                if engine == "cp_sat_counts":
                    _lineups, _diag = solve_cpsat_iterative_counts(players_adj, base_constraints, seed, site)
                else:
                    _lineups, _diag = solve_cpsat_iterative(players_adj, base_constraints, seed, site)
            except Exception:
                return []
            if not _lineups:
                return []
        ...
        print(f"🎯 Lambda search result: λ={lam_used:.3f}, achieved {achieved_pct:.1%} off-optimal (CP-SAT)")
        ...
        id_diagnostics["ownership_penalty"].update({
            ...
            "target_offoptimal_pct": float(constraints.ownership_penalty.target_offoptimal_pct),
    ...
    if engine == "cp_sat":
        ...
        try:
            ...
            if pen_cfg and pen_cfg.enabled and lineups:
                ...
                if isinstance(base_constraints.cp_sat_params, dict):
                    ...
                    params['max_time_seconds'] = max(0.25, min(mt, 1.0))
                    base_constraints.cp_sat_params = params
                baseline_lineups, _baseline_diag = solve_cpsat_iterative(players, base_constraints, seed, site)
                if baseline_lineups:
                        ...
                        for lu in lineups:
                            ...
                            for pl in lu.players:
                                if pl.own_proj is None:
                                    continue
                                own_pct = pl.own_proj / 100.0 if pl.own_proj > 1.0 else float(pl.own_proj)
        ...
        print(f"🚀 Using CP-SAT Counts solver engine (PRP-14) (seed={seed}, site={site})")
        try:
        ...
        lineups, diagnostics = solve_cpsat_iterative_counts(players, constraints, seed, site)
        # Merge DK ID diagnostics into solver diagnostics for consistency
        if not isinstance(diagnostics, dict):
            diagnostics = {}
        diagnostics.update(id_diagnostics)
    ...
    ownership_penalty = constraints.ownership_penalty
    if (ownership_penalty and ownership_penalty.enabled and 
        ownership_penalty.mode == "by_percent"):
        ...
        def build_and_solve_fn(coeffs: Dict[str, float]) -> List[str]:
            ...
            lineup_vars = solve_problem(temp_problem, temp_lp_variables, temp_var_index, seed)
        ...
        print(f"🎯 Lambda search result: λ={lam_used:.3f}, achieved {achieved_pct:.1%} off-optimal")
    ...
    problem, var_index, lp_variables, base_objective = build_problem(players, constraints, site)
    ...
    for i in range(constraints.N_lineups):
        try:
            ...
            if constraints.randomness_pct > 0:
                apply_randomness_to_objective(problem, lp_variables, var_index, constraints, seed + i)
            else:
    ...
    try:
        ...
        if pen_cfg and pen_cfg.enabled:
            ...
            try:
                if pen_cfg.mode == 'by_points' and 'target_offoptimal_pct' in pen_cfg.__dict__:
                    # no-op; legacy
                    pass
            except Exception:
                pass
            ...
            if _os.environ.get('DFS_DEBUG'):
                try:
                    ...
                    for p in players[:10]:
                        ...
                        snap.append({'name': p.get('name'), 'own_proj': float(op), 'g': float(gv)})
                    ...
                    if lineups:
                        ...
                        for pl in lu0.players:
                            if pl.own_proj is None:
                                continue
                            op = pl.own_proj / 100.0 if pl.own_proj > 1.0 else float(pl.own_proj)
...
def optimize_dk_strict(
    ...
    try:
        ...
        lineups, diagnostics = optimize_with_diagnostics(projections_df, constraints, seed, site, None, engine)
        ...
        if norm_info and norm_info.get("ownership"):
            diagnostics.setdefault("normalization", {})["ownership"] = norm_info["ownership"]
        ...
        raise OptimizerError(
            ...
            details={"projections_path": projections_path, "player_ids_path": player_ids_path}
        )
...
def optimize_to_dk_csv(
    ...
    if not lineups:
        raise OptimizerError(
            ...
            details={"generated_lineups": 0, "id_match_rate": id_diagnostics.get("success_rate")}
        )
    ...
    if errors:
        ...
        preview = "; ".join(
            [f"Lineup {e.get('lineup_id')}: {', '.join(e.get('errors', [])[:2])}" for e in errors[:3]]
        )
        raise OptimizerError(
...
def optimize_dk_strict_to_csv(
    ...
    lineups, diagnostics = optimize_dk_strict(projections_path, constraints, seed, player_ids_path, engine)
    ...
    if errors:
        error_summary = "; ".join([f"Lineup {e.get('lineup_id')}: {', '.join(e.get('errors', [])[:2])}" for e in errors[:3]])
        raise OptimizerError(
            code="DK_STRICT_VALIDATION_FAILED", 
            message=f"DK-Strict validation failed for {len(errors)} lineups",
...
def attach_player_ids_if_available(df: pd.DataFrame, site: SiteType, ids_df: Optional[pd.DataFrame]) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """
    ...
    if missing_cols:
        diagnostics["errors"].append(f"Missing required columns in player IDs: {missing_cols}")
        return df, diagnostics

    print(f"🔍 DK ID Matching: {len(df)} players in projections, {len(ids)} IDs available")

    # Enhanced normalization for matching
    tmp = df.copy()
    tmp["__name_norm"] = tmp["name"].apply(normalize_player_name)
    tmp["__team_norm"] = tmp["team"].astype(str).str.upper().map(lambda t: TEAM_REPLACEMENT_DICT.get(t, t))

    ids = ids.rename(columns={"ID": "dk_id", "Name": "name_ids", "TeamAbbrev": "team_ids", "Position": "pos_ids"})
    ids["__name_norm"] = ids["name_ids"].apply(normalize_player_name)
    ids["__team_norm"] = ids["team_ids"].astype(str).str.upper().map(lambda t: TEAM_REPLACEMENT_DICT.get(t, t))
    ids["__pos_list"] = ids["pos_ids"].astype(str).str.split("/")

    # Exact merge on normalized name + team
    merged = tmp.merge(
        ids[["__name_norm", "__team_norm", "dk_id", "__pos_list", "name_ids", "team_ids"]],
    ...
    if initial_matches < len(df) and FUZZY_MATCHING_AVAILABLE:
        ...
        if not fuzzy_matches.empty:
            ...
            diagnostics["warnings"].append(f"Applied {fuzzy_match_count} fuzzy name matches")
    ...
    if position_failures > 0:
        failed_players = merged.loc[mask_bad_pos, ["name", "position", "name_ids", "pos_ids"]].head(5)
        diagnostics["warnings"].append(f"Position validation failed for {position_failures} players")
    ...
    if final_matches == 0:
        diagnostics["errors"].append("CRITICAL: Zero DK IDs matched - lineups will not be DK-compatible")
        if diagnostics["failed_name_matching"] > 0:
            diagnostics["errors"].append("• Check player name formatting in projections vs player_ids.csv")
            diagnostics["errors"].append("• Verify team abbreviations match between files")
        if len(ids) == 0:
            diagnostics["errors"].append("• Player IDs file is empty")
    elif final_matches < len(df) * 0.95:  # Less than 95% match rate
        diagnostics["warnings"].append(f"Low match rate: {final_matches}/{len(df)} ({diagnostics['success_rate']:.1f}%)")
    ...
    print(f"📊 Match Results: {final_matches}/{len(df)} ({diagnostics['success_rate']:.1f}%) successful")
    if diagnostics["failed_name_matching"] > 0:
        print(f"❌ Name/Team matching failed: {diagnostics['failed_name_matching']} players")
    if diagnostics["failed_position_validation"] > 0:
        print(f"⚠️  Position validation failed: {diagnostics['failed_position_validation']} players")

    # Clean up temporary columns
    result_df = merged.drop(columns=["__name_norm", "__team_norm", "__pos_list", "name_ids", "team_ids"], errors="ignore")
...
def convert_projections_to_players(df: pd.DataFrame, proj_min: float) -> List[Dict]:
    ...
    for _, row in df.iterrows():
        ...
        if pd.notna(dk_id):
            ...
            player_id = (str(row.get('player_id', '')).strip() or 
                         str(row.get('fd_id', '')).strip() or
                         f"{name}_{team}_{salary}_{hash(name) & 0xffff}")
        ...
        player = {
            ...
            'dk_id': str(int(float(dk_id))) if pd.notna(dk_id) else None  # Real DK ID if available
        }
...
def build_problem(players: List[Dict], constraints: Constraints, site: SiteType, override_coeffs: Optional[Dict[str, float]] = None) -> Tuple[plp.LpProblem, Dict, Dict, plp.LpAffineExpression]:
    """
    ...
    if override_coeffs is not None:
    ...
    elif (ownership_penalty and ownership_penalty.enabled and 
          ownership_penalty.mode == "by_points" and ownership_penalty.weight_lambda > 0):
        ...
        for var_key, variable in lp_variables.items():
            ...
            if player.get('own_proj') is not None:
                ...
                penalty = ownership_penalty.weight_lambda * _g_curve_penalty(own_pct, ownership_penalty)
...
def apply_randomness_to_objective(problem: plp.LpProblem, lp_variables: Dict, 
                                  var_index: Dict, constraints: Constraints, iteration_seed: int):
    ...
    ownership_penalty = constraints.ownership_penalty
    use_penalty = (
        ownership_penalty and ownership_penalty.enabled and 
        ownership_penalty.mode == "by_points" and ownership_penalty.weight_lambda > 0
    )
    for var_key, variable in lp_variables.items():
        player = var_index[var_key]
        if player.get('stddev') and constraints.randomness_pct > 0:
            randomized_proj = np.random.normal(
                player['proj'], 
                player['stddev'] * constraints.randomness_pct / 100
            )
        else:
...
def add_salary_constraints(problem: plp.LpProblem, lp_variables: Dict, 
                          var_index: Dict, constraints: Constraints, site: SiteType):
...
def add_position_constraints(problem: plp.LpProblem, lp_variables: Dict, 
                           var_index: Dict, site: SiteType):
    ...
    if site == "dk":
        ...
        for pos in positions:
            position_sum = plp.lpSum(
                variable for var_key, variable in lp_variables.items()
                if var_key[1] == pos
            )
...
def add_team_constraints(problem: plp.LpProblem, lp_variables: Dict,
                        var_index: Dict, constraints: Constraints, site: SiteType):
    ...
    for team in teams:
        ...
        if global_limit is not None:
            ...
            if not (site == "fd" and global_limit >= 4):
                problem += plp.lpSum(team_variables) <= global_limit, f"Team_{team}_global_{global_limit}"


def add_player_constraints(problem: plp.LpProblem, lp_variables: Dict,
                          var_index: Dict, constraints: Constraints):
    ...
    for rule in constraints.at_least:
        ...
        if matching_variables:
            problem += plp.lpSum(matching_variables) >= rule.count, f"AtLeast_{rule.count}_{hash(tuple(rule.players))}"
    ...
    for rule in constraints.at_most:
        ...
        if matching_variables:
            problem += plp.lpSum(matching_variables) <= rule.count, f"AtMost_{rule.count}_{hash(tuple(rule.players))}"


def solve_problem(problem: plp.LpProblem, lp_variables: Dict, var_index: Dict, seed: int) -> Optional[List[Tuple]]:
    """
    ...
    try:
        ...
        solver = plp.PULP_CBC_CMD(msg=False, timeLimit=9, options=[f"randomSeed {seed}"])
...
def convert_vars_to_lineup(var_keys: List[Tuple], var_index: Dict, 
                          players: List[Dict], lineup_id: int) -> Lineup:
...
def add_uniqueness_constraint(problem: plp.LpProblem, lp_variables: Dict, selected_vars: List[Tuple],
                             unique_count: int, constraint_name: str):
    ...
    player_vars = [variable for var_key, variable in lp_variables.items() 
                   if var_key[0] in selected_player_ids]
    
    if player_vars:
        # Must have at least unique_count different players
        problem += (plp.lpSum(player_vars) <= len(selected_player_ids) - unique_count, constraint_name)
...
class NBA_Optimizer:
    ...
    def __init__(self, site=None, num_lineups=0, num_uniques=1):
        ...
        print("Warning: NBA_Optimizer class is deprecated. Use optimize() function instead.")
    ...
    try:
        ...
        id_diagnostics.setdefault("normalization", {}).setdefault("ownership", {})["run_id"] = RUN_ID
        ...
        try:
            ...
            if pen_cfg and getattr(pen_cfg, 'enabled', False):
                mx = float(projections_df["own_proj"].max()) if "own_proj" in projections_df.columns else float("inf")
                id_diagnostics["normalization"]["ownership"]["own_proj_max_pre_solve"] = mx
        ...
        try:
            ...
            rename_map = {"FPTS": "FPts", "fpts": "FPts", "proj": "FPts", "projection": "FPts", "Projection": "FPts"}
            df_contract = df_contract.rename(columns={k: v for k, v in rename_map.items() if k in df_contract.columns})
            ...
            contract_required = {"player_id", "name", "team", "position", "salary", "FPts", "own_proj"}
            ...
            mx = float(df_contract["own_proj"].max()) if "own_proj" in df_contract.columns else float("inf")
            assert mx <= 1.000001, f"[CONTRACT] own_proj not normalized (max={mx})"
            # Contract hash over deterministic subset
            cols_for_hash = ["player_id", "FPts", "own_proj", "salary", "team", "position"]
            key = df_contract[cols_for_hash].sort_values("player_id").to_csv(index=False)
            ...
            try:
                id_diagnostics["solver_contract"]["csv"] = df_contract.to_csv(index=False)
            ...
            try:
                if os.environ.get("DFS_WRITE_CONTRACT_ARTIFACTS", "0") in ("1", "true", "True"):
            ...
            id_diagnostics.setdefault("normalization", {}).setdefault("ownership", {})["projections_used_hash"] = _df_hash(df_contract)
            print(f"[CONTRACT] run={RUN_ID} rows={df_contract.shape[0]} own_max={mx:.3f} hash={contract_hash[:12]}")
